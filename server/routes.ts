...
<Response>
  <Say voice="alice">Hello! You've reached Pearl AI voice assistant. How can I help you today?</Say>
  <Record maxLength="300" playBeep="true" transcribe="true" transcribeCallback="${process.env.REPLIT_DOMAINS?.split(',')[0] || 'http://localhost:5000'}/api/twilio/transcribe"/>
...
<Response>
  ...
  // Mount WebSocket on /ws path to avoid conflicts with Vite HMR
  const wss = new WebSocketServer({ noServer: true });

  httpServer.on('upgrade', (request, socket, head) => {
    const pathname = new URL(request.url || '', `http://${request.headers.host}`).pathname;
    
    // Only handle /ws path for our application WebSocket
    if (pathname === '/ws') {
